...
__global__ void reductionKernel(
    ...
    // The reduction loop : squeezes all partial sums to a single one and going till our thread_index is 0;
...
extern "C" void solution(
    ...
    // Copy sum to host and compute norm
    float sum;
    cudaMemcpy(&sum, partialSums, sizeof(float), cudaMemcpyDeviceToHost);
    //After reduction is done; we simply pass the reduced sum of squares through a sqrt and then pass it to norm kernel :) 
    float norm = sqrtf(sum);

    // Normalization kernel
    normalizeKernel<<<blocksPerGrid, threadsPerBlock>>>(x, y, norm, size);
